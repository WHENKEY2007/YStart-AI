...
export async function interviewTurn(startup, history) {
  ...
  const system = `You are the Founder Interview Agent of YStart-AI. You are interviewing the founder of "${startup.name}" to deeply understand their concept: "${startup.idea}".

STYLE RULES:
- Always tailor your questions directly to their specific startup concept ("${startup.name}": ${startup.idea}).
...
- If no messages yet (turn 1), ask a direct question about who specifically experiences the problem they described in their idea.
- The founder has answered ${answers} questions so far. Target 8-10 total answers across: target user, problem severity, existing alternatives, value proposition, pricing/revenue, first 100 users, and key risks.
...
Return JSON: {"question": string, "done": boolean, "progress": number, "topic": string}`
  const user = `STARTUP NAME: ${startup.name}\nIDEA: ${startup.idea}\n\nINTERVIEW SO FAR:\n${transcript || `(no messages yet — ask a targeted opening question specifically about the target user or core problem for ${startup.name}: ${startup.idea})`}`
  const fallbackQ = `Welcome to YStart-AI! For ${startup.name}, who is your specific target customer and what is the biggest pain point you solve for them?`
  try {
    const out = await aiJSON(system, user)
    return {
      question: str(out.question) || fallbackQ,
      done: answers >= 10 ? true : Boolean(out.done),
      progress: clampInt(out.progress, 0, 100, Math.min(95, Math.round((answers / 10) * 100))),
      topic: str(out.topic),
    }
  } catch (e) {
    console.warn('interviewTurn fallback used:', e.message)
    return {
      question: fallbackQ,
      done: answers >= 10,
      progress: Math.min(95, Math.round((answers / 10) * 100)),
